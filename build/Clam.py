...
def on_button_scan():
    ...
    clamav_output = subprocess.run(["clamscan", "-r", "/home/roxy/Documents/sca/cobalt"], capture_output=True, text=True)
    ...
    if "FOUND" in clamav_output.stdout:
        # Show a confirmation dialog
        result = messagebox.askyesno("Confirmation", "There are infected files found. Do you want to remove them?")

        # If the user confirms, run ClamAV again with the --remove option
        if result == True:
            clamav_output = subprocess.run(["clamscan", "-r", "--remove", "/home/roxy/Documents"], capture_output=True,
                                           text=True)
